...
*View interactive version <a href="https://api.gamesparks.net/#challengedeclinedmessage" target="_apidocs">here</a>*
...
<h3>C#</h3>
```cs
	ChallengeDeclinedMessage.Listener = (message) => {
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeDeclinedMessage",
		function (message:ChallengeDeclinedMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeDeclinedMessage:^(GSChallengeDeclinedMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeDeclinedMessageListener(
		new GSEventConsumer<ChallengeDeclinedMessage>() {
			public void onEvent(ChallengeDeclinedMessage event) {
...
```cpp
	...
	...
	void OnChallengeDeclinedMessage(GS& gsInstance, const ChallengeDeclinedMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeDeclinedMessage);
```
