...
*View interactive version <a href="https://api.gamesparks.net/#challengechatmessage" target="_apidocs">here</a>*
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeChatMessage",
		function (message:ChallengeChatMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeChatMessage:^(GSChallengeChatMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeChatMessageListener(
		new GSEventConsumer<ChallengeChatMessage>() {
			public void onEvent(ChallengeChatMessage event) {
...
```cpp
	...
	...
	void OnChallengeChatMessage(GS& gsInstance, const ChallengeChatMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeChatMessage);
```
