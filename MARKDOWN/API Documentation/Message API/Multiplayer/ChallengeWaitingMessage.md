...
*View interactive version <a href="https://api.gamesparks.net/#challengewaitingmessage" target="_apidocs">here</a>*
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeWaitingMessage",
		function (message:ChallengeWaitingMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeWaitingMessage:^(GSChallengeWaitingMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeWaitingMessageListener(
		new GSEventConsumer<ChallengeWaitingMessage>() {
			public void onEvent(ChallengeWaitingMessage event) {
...
```cpp
	...
	...
	void OnChallengeWaitingMessage(GS& gsInstance, const ChallengeWaitingMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeWaitingMessage);
```
