---
src: /API Documentation/Message API/Multiplayer/ChallengeDrawnMessage.md
---

# ChallengeDrawnMessage

<a href="https://api.gamesparks.net/#challengedrawnmessage" target="_gsapi">View interactive version here</a>
...
<h3>C#</h3>
```cs
	ChallengeDrawnMessage.Listener = (message) => {
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeDrawnMessage",
		function (message:ChallengeDrawnMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeDrawnMessage:^(GSChallengeDrawnMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeDrawnMessageListener(
		new GSEventConsumer<ChallengeDrawnMessage>() {
			public void onEvent(ChallengeDrawnMessage event) {
...
```cpp
	...
	...
	void OnChallengeDrawnMessage(GS& gsInstance, const ChallengeDrawnMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeDrawnMessage);
```
