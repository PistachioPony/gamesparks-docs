...
<a href="https://api.gamesparks.net/#challengeturntakenmessage" target="_gsapi">View interactive version here</a>
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeTurnTakenMessage",
		function (message:ChallengeTurnTakenMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeTurnTakenMessage:^(GSChallengeTurnTakenMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeTurnTakenMessageListener(
		new GSEventConsumer<ChallengeTurnTakenMessage>() {
			public void onEvent(ChallengeTurnTakenMessage event) {
...
```cpp
	...
	...
	void OnChallengeTurnTakenMessage(GS& gsInstance, const ChallengeTurnTakenMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeTurnTakenMessage);
```
