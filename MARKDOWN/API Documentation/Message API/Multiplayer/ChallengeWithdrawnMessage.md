---
src: /API Documentation/Message API/Multiplayer/ChallengeWithdrawnMessage.md
---

# ChallengeWithdrawnMessage

*View interactive version <a href="https://api.gamesparks.net/#challengewithdrawnmessage" target="_apidocs">here</a>*
...
<h3>C#</h3>
```cs
	ChallengeWithdrawnMessage.Listener = (message) => {
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeWithdrawnMessage",
		function (message:ChallengeWithdrawnMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeWithdrawnMessage:^(GSChallengeWithdrawnMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeWithdrawnMessageListener(
		new GSEventConsumer<ChallengeWithdrawnMessage>() {
			public void onEvent(ChallengeWithdrawnMessage event) {
...
```cpp
	...
	...
	void OnChallengeWithdrawnMessage(GS& gsInstance, const ChallengeWithdrawnMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeWithdrawnMessage);
```
