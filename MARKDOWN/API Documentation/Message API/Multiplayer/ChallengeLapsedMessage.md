...
*View interactive version <a href="https://api.gamesparks.net/#challengelapsedmessage" target="_apidocs">here</a>*
...
<h3>C#</h3>
```cs
	ChallengeLapsedMessage.Listener = (message) => {
...
```actionscript
	gs.getMessageHandler().setHandler(
		".ChallengeLapsedMessage",
		function (message:ChallengeLapsedMessage):void {
...
### Objective-C
```objectivec
	[listener onGSChallengeLapsedMessage:^(GSChallengeLapsedMessage* message) {
...
### Java
```java
	gs.getMessageHandler().setChallengeLapsedMessageListener(
		new GSEventConsumer<ChallengeLapsedMessage>() {
			public void onEvent(ChallengeLapsedMessage event) {
...
```cpp
	...
	...
	void OnChallengeLapsedMessage(GS& gsInstance, const ChallengeLapsedMessage& message)
	{
	...
	...
	GS.SetMessageListener(OnChallengeLapsedMessage);
```
