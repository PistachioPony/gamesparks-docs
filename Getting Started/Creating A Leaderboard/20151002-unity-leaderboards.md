...
You can see more details about Events [here](https://docs.gamesparks.net/developer-portal/events).
...
Next you'll need to create your Leaderboard. You can do this by going to the *Configurator* panel in the GameSparks Portal and selecting the *Leaderboards* tab. Add a new Leaderboard by clicking on the *[+]* button. Most of the details you see here can be left as default but you can get more information about these attributes [here](https://docs.gamesparks.net/blog/2015/05/26/leaderboards-2/). The important variables for us are the *Short Code*, *Name* and *Description*. If this is the first event you have created in your own game you will see that the *Submit Score* Event already pops into that field. This is because it is the only applicable Event you have in your game at the moment.
...
Now you are ready to send some scores to your Leaderboard, but before you go into Unity, you ahould test this Leaderboard in the *Test Harness*. After authenticating yourself as a Player, find your Event in the *LogEvent* tab, enter a score and send the Event. Immediately after you send the request, you will see orange message appear in the inspector on the left. This is a High Score Message and when you go back into Unity you are going to setup an Event listener to execute some code whenever a player gets this Message.
...
You have already seen how to send a logEventRequest, in Unity, in the previous tutorial on Cloud Code. You can check out [here](https://docs.gamesparks.net/developer-portal/cloud-code/unity-cloud-code "Unity Cloud Code"). The code is as follows:

``{r}new GameSparks.Api.Requests.LogEventRequest().SetEventKey("SUBMIT_SCORE").SetEventAttribute("SCORE", "1234").Send((response) => {
